...
Original file is located at
    https://colab.research.google.com/drive/1GQRqh4YSZXowm8aq4tJNN4-bZTVQxO7m
"""
...
BATCH_SIZE = 10000
N_BATCHES = 1
TARGET_REPLACE_STEPS = 500
DELAY_ACTOR_STEPS = 1000
DELAY_CRITIC_STEPS = 500
polyak = 0.23
t_episode = 300
γ = 0.998
start_timesteps = 1e5
...
def picked_and_lifted_reward(box_pos):
    x,y,z = box_pos[:,0:1],box_pos[:,1:2],box_pos[:,2:3]
    return (z>0.5).float() - 1.
        
def touched_reward(box1_pos, box2_pos):
    x1,y1,z1 = box1_pos[:,0:1],box1_pos[:,1:2],box1_pos[:,2:3]
    x2,y2,z2 = box2_pos[:,0:1],box2_pos[:,1:2],box2_pos[:,2:3]
...
box = world.load_box(position=(0.5,0,0.1),dimensions=(0.05,0.05,0.05),mass=0.1,color=[0,0,1,1])
boxv = world.load_visual_box(position=(0.5,0,0.3),dimensions=(0.05,0.05,0.05),color=[1,0,0,1])
...
states = LinkWorldVelocityState(manipulator, link_ids=end_effector) + LinkWorldPositionState(manipulator, link_ids=other_links)  + LinkWorldVelocityState(manipulator, link_ids=other_links) + LinkWorldPositionState(manipulator, link_ids=end_effector) + PositionState(box,world) + PositionState(boxv, world)
STATES_SHAPE = [i.shape[0] for i in states()]
N_STATES = sum(STATES_SHAPE)
...
for i in count(start=1):
    ...
    # get end effector position and move box below it
    end_effector_pos = states[-3]()[0]
    end_effector_pos[-1] -= 0.1 # in case of collision
    world.move_object(box,end_effector_pos)

    # randomly reset target visual box position
    x = np.random.choice([np.random.uniform(-0.5,-0.1), np.random.uniform(0.1,0.5)])
    y = np.random.choice([np.random.uniform(-0.5,-0.1), np.random.uniform(0.1,0.5)])
    z = np.random.uniform(0.0,0.5)
    world.move_object(boxv,[x,y,z])
    # run an episode
